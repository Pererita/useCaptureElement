...
Recomendamos instalar el paquete utilizando **pnpm** para un manejo eficiente del almacenamiento:

```sh
pnpm add use-capture-element
```

O si utilizas npm o yarn:
...
¡Las contribuciones son muy bienvenidas! Para desarrollar localmente en este proyecto estructurado en Monorepo:

1. Clonar el repositorio y correr la instalación:
   ```sh
   pnpm install
   ```
2. Correr la aplicación de demostración interactiva:
   ```sh
   pnpm dev:example
   ```
...
_Nota: Asegúrate de que todos los mensajes de commit sigan el estándar de **Conventional Commits 1.0.0** y estén redactados en **español**._
